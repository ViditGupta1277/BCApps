...
codeunit 6790 "WHT Employee Calculation"
{
    var
        ...
        // EmployeeWithholdingExemptMsg: Label 'Employee %1 is exempt from withholding tax.', Comment = '%1 = Employee No.';
    ...
    // ThresholdNotExceededMsg: Label 'Withholding tax threshold not exceeded for posting setup %1/%2.', Comment = '%1 = Bus. Post. Group, %2 = Prod. Post. Group';
    ...
    begin
        ...
        if not PostingSetup.Get(GenJnlLine."Wthldg. Tax Bus. Post. Group", GenJnlLine."Wthldg. Tax Prod. Post. Group") and IsExpenseCategorySingleTax(GenJnlLine) then
    ...
    procedure ResolveWHTGroupComponents(GenJnlLine: Record "Gen. Journal Line"; var WHTGroupCode: Code[20]): Boolean
    var
    ...
    begin
        ...
        if not ExpenseCategory.Get(GenJnlLine."Expense Category") then
            exit(false);

        if not WHTGroup.Get(ExpenseCategory."Withholding Group Code") then
            exit(false);

        WHTGroupCode := ExpenseCategory."Withholding Group Code";
        exit(true);
    end;
            ...
            ThresholdBase::"Category Period":
                begin
                    AccumulatedAmount := GetAccumulatedAmount(
                        ...
                        PostingSetup."Threshold Category Code",
    ...
    local procedure GetAccumulatedAmount(EmployeeNo: Code[20]; BusPostGroup: Code[20]; ProdPostGroup: Code[20]; CategoryCode: Code[20]; ThresholdPeriod: Enum "WHT Threshold Period Type"; PostingDate: Date): Decimal
    var
        ThresholdAccumulator: Record "WHT Threshold Accumulator";
        PeriodStartDate: Date;
        PeriodEndDate: Date;
    begin
        ...
        ThresholdAccumulator.SetRange("Expense Category Code", CategoryCode);
