...
codeunit 6791 "Wthldg Tax Empl. Subscribers"
{
    ...
    begin
        ...
        // When threshold not yet exceeded, WHT is zero but the base must still be
        // accumulated so future transactions can evaluate the running total correctly.
    ...
    begin
        ...
        // Post to Payable WHT Account (debit) — reduces the payment to employee
    ...
    local procedure OnAfterInitGLRegisterForEmployee(var GLRegister: Record "G/L Register")
    begin
        // This event is already handled by the vendor subscriber to set
        // "From Withholding Tax Entry No.". Employee entries share the same
        // WHT Entry table, so no separate handling is needed.
    end;
